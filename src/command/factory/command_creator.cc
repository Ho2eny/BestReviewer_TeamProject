...
UserCommandCreator::CreateCommand(const CommandType &type,
                                  const std::string description) const {
  ReceiverCreator *userReceiverCreator = new UserReceiverCreator(cache_);
  if (type == CommandType::kLogin)
    return move(make_unique<Login>(type, description,
                                   userReceiverCreator->CreateAReceiver(type)));

  if (type == CommandType::kSignup)
    return move(make_unique<Signup>(
        type, description, userReceiverCreator->CreateAReceiver(type)));

  if (type == CommandType::kLogout)
    return move(make_unique<Logout>(
        type, description, userReceiverCreator->CreateAReceiver(type)));
...
std::unique_ptr<Command>
RoomCommandCreator::CreateCommand(const CommandType &type,
                                  const std::string description) const {
  ReceiverCreator *roomReceiverCreator = new RoomReceiverCreator(cache_);

  if (type == CommandType::kListRooms)
    return move(make_unique<ListChatRooms>(
        type, description, roomReceiverCreator->CreateAReceiver(type)));

  if (type == CommandType::kCreateRoom)
    return move(make_unique<CreateChatRoom>(
        type, description, roomReceiverCreator->CreateAReceiver(type)));
